...
export function TestConsole({
  ...
  return (
    <section className="flex h-[calc(100dvh-8rem)] max-h-[760px] flex-col rounded-lg border border-border bg-muted/10">
      ...
      {dirty ? (
        <p className="border-b border-amber-500/20 bg-amber-500/10 px-3 py-1.5 text-[11px] text-amber-600 dark:text-amber-400">
        ...
        {turns.map((t, i) =>
          t.role === "user" ? (
            <div key={i} className="flex justify-end">
              <div className="max-w-[85%] rounded-2xl rounded-br-md bg-foreground px-3 py-2 text-[13px] leading-relaxed whitespace-pre-wrap text-background">
                {t.content}
              </div>
            </div>
          ) : (
