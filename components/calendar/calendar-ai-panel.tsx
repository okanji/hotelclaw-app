...
export function CalendarAiPanel({
  ...
  return (
    <div
      className={cn(
        "absolute inset-y-0 right-0 z-40 flex w-96 flex-col border-l border-border bg-card shadow-xl transition-transform duration-200",
    ...
    >
      <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
        <div>
          <div className="text-sm font-medium">Calendar assistant</div>
          <div className="text-[11px] text-muted-foreground">
            Powered by Liveblocks AI
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="flex w-full flex-col gap-1.5">
        {suggestions.map((s) => (
          <button
            ...
            type="button"
            onClick={() => send(s)}
            className="rounded-md border border-border bg-background px-3 py-2 text-left text-xs text-foreground/80 transition-colors hover:border-foreground/30 hover:bg-accent/40"
          >
            {s}
          </button>
        ))}
