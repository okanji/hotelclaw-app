...
export function ProjectsTableView({
  ...
  return (
    <div className="h-full overflow-y-auto px-8 py-2 sm:px-14">
      {/* Header */}
      <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-border/60 bg-background py-2 text-[0.6875rem] font-medium tracking-wider text-muted-foreground uppercase">
        {COLUMNS.map((col, i) => {
          ...
          return (
            <li key={p.id} className="group/row">
              <Link
                ...
                className="flex items-center gap-3 rounded-md py-2.5 text-[0.8125rem] tracking-tight transition-colors hover:bg-muted focus-visible:bg-muted focus-visible:outline-none"
              >
                {/* Project */}
                ...
                <div className="w-28 shrink-0">
                  <span
                    className={cn(
                      "inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[0.75rem] font-medium",
                      status.soft,
                      status.text,
                    )}
                  >
                ...
                <div className="flex w-[200px] shrink-0 items-center gap-2.5">
                  ...
                  />
                  <span className="w-8 shrink-0 text-right text-[0.75rem] font-medium tabular-nums text-foreground">
                    {pct}%
                  </span>
                ...
                {/* Start */}
                <div className="hidden w-16 shrink-0 text-right text-[0.75rem] tabular-nums text-muted-foreground xl:block">
                  {shortDate(p.start_date)}
                </div>

                {/* Target */}
                <div className="w-16 shrink-0 text-right text-[0.75rem] tabular-nums text-muted-foreground">
