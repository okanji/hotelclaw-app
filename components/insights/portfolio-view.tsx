      ...
      >
        <div className="flex items-baseline justify-between gap-3">
          <span className="flex min-w-0 items-baseline gap-2">
            <span className="truncate text-[0.8125rem] font-medium text-foreground group-hover:underline">
              {row.name}
            </span>
        ...
        {row.paceReasons.length > 0 ? (
          <p className="text-[0.75rem] text-pretty text-amber-600 dark:text-amber-500">
            {row.paceReasons.join(" · ")}
          </p>
        ) : null}
        {aiNote ? (
          <p className="flex items-start gap-1.5 text-[0.75rem] text-pretty text-muted-foreground">
