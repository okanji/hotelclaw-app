    ...
    return (
      <div className="pointer-events-none absolute inset-x-0 bottom-4 z-20 flex justify-center">
        <button
          ...
          className="pointer-events-auto flex items-center gap-2 rounded-full border border-border bg-background/95 px-4 py-2 text-[0.8125rem] font-medium text-foreground shadow-lg backdrop-blur transition-colors hover:bg-muted"
        >
      ...
      <div className="pointer-events-auto flex w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-border bg-background/95 shadow-xl backdrop-blur">
          ...
          {turns.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-1.5">
                {SUGGESTIONS.map((s) => (
                  <button
                    ...
                    type="button"
                    onClick={() => send(s)}
                    className="rounded-full border border-border bg-muted/40 px-3 py-1 text-[0.75rem] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                  >
                    {s}
                  </button>
                ))}
            ...
            turns.map((t, i) =>
              t.role === "user" ? (
                <p
                  key={i}
                  className="ml-8 self-end rounded-lg bg-muted px-3 py-1.5 text-[0.8125rem] text-foreground"
                >
                  {t.content}
                </p>
              ) : (
                <div key={i} className="flex flex-col gap-1">
                  <div
                    className={cn(
                      ...
                      "text-[0.8125rem] leading-relaxed text-foreground [&_li]:my-0.5 [&_p]:my-1",
                    )}
                  >
                  ...
                  {turns[i - 1]?.role === "user" ? (
                    <button
                      ...
                      className="flex w-fit items-center gap-1 rounded-md px-1.5 py-0.5 text-[0.6875rem] font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
        ...
        >
          <textarea
            ...
            disabled={busy}
            className="max-h-28 min-h-9 flex-1 resize-none rounded-md border border-input bg-transparent px-3 py-2 text-[0.8125rem] outline-none placeholder:text-muted-foreground focus-visible:ring-1 focus-visible:ring-ring disabled:opacity-50"
          />
          <button
            type="submit"
            aria-label="Send"
