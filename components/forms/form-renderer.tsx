      ...
      {fields.map((field) =>
        field.type === "section" ? (
          ...
          <div key={field.id} className={cn("space-y-2", !roomy && "space-y-1.5")}>
            <Label
              htmlFor={`ff-${field.id}`}
              className={cn("gap-0.5", roomy ? "text-[0.9375rem] font-semibold" : "text-sm")}
            >
              {field.label}
              {field.required ? <span className="text-destructive">*</span> : null}
            </Label>
            {field.description ? (
              <p className="text-[0.8125rem] text-muted-foreground">{field.description}</p>
            ) : null}
            <FormFieldInput
              field={field}
    ...
    case "phone":
      return (
        <Input
          ...
          disabled={disabled}
          autoFocus={autoFocus}
          className={cn(large && "h-11 rounded-lg px-3.5 text-[0.9375rem]")}
        />
      );
    case "long_text":
      return (
        <Textarea
          ...
          autoFocus={autoFocus}
          rows={large ? 4 : 3}
          className={cn(large && "rounded-lg px-3.5 py-3 text-[0.9375rem]")}
        />
      );
    case "number":
      return (
      ...
      return (
        <div className="relative w-fit">
          ...
          <Input
            ...
            disabled={disabled}
            autoFocus={autoFocus}
            className={cn("pl-9", large && "h-11 rounded-lg text-[0.9375rem]")}
          />
        </div>
      );
    case "select": {
        ...
        <div className="relative">
          <select
            ...
            className={cn(
              ...
              large ? "h-11 px-3.5 text-[0.9375rem]" : "h-8 px-2.5 text-sm",
    ...
    <div className="space-y-2">
      ...
      >
        {uploading ? (
          ...
          <span className={cn("text-muted-foreground", large ? "text-[0.9375rem]" : "text-sm")}>
