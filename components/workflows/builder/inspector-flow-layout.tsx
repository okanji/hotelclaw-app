  ...
  return (
    <div className="flex gap-3">
      <div className="flex w-7 shrink-0 flex-col items-center">
        <span
          className={cn(
            "flex size-7 items-center justify-center rounded-full border bg-background text-[0.6875rem] font-semibold tabular-nums",
            step === "+"
...
export function InspectorAdvancedCollapse({ children }: { children: ReactNode }) {
  return (
    <details className="mt-6 rounded-lg border border-border/50 bg-muted/[0.03]">
      <summary className="cursor-pointer list-none px-3 py-2.5 text-[0.8125rem] font-medium text-muted-foreground select-none hover:text-foreground [&::-webkit-details-marker]:hidden">
        Advanced
      </summary>
