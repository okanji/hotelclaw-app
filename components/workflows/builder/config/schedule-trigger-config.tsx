    ...
    <div className="space-y-4">
      {isCron ? (
        <>
                ...
                {DAY_LABELS.map((d, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "h-8 w-10 rounded-md border text-[0.8125rem] font-medium transition-colors",
                        on
          ...
          {parsed.freq === "custom" ? (
            <Field label="Schedule pattern (cron)">
              ...
              <Hint>
                ...
                <code className="rounded bg-muted px-1 py-0.5 text-[0.6875rem]">0 9 * * 1</code>{" "}
                for Mondays at 9:00.
              </Hint>
            </Field>
          ) : null}
          ...
          {parsed.freq !== "custom" ? (
            <p className="rounded-md bg-muted/40 px-3 py-2 text-[0.8125rem] text-foreground/85">
...
const SELECT_CLASS =
  "h-9 w-full rounded-lg border border-input bg-transparent px-2.5 text-[0.8125rem] outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 dark:bg-input/30";

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block space-y-1.5">
      <span className="text-[0.8125rem] font-medium text-foreground/80">{label}</span>
      {children}
    </label>
  );
}
...
function Hint({ children }: { children: React.ReactNode }) {
  return <span className="block text-[0.75rem] leading-relaxed text-muted-foreground">{children}</span>;
}
