...
export default defineEval({
  ...
  async test(t) {
    // BKG-WTMU03 is seeded CONFIRMED and stays confirmed: the park below is
    // never approved, so the eval is re-runnable without reseeding.
    await t.send(
      "Refund booking BKG-WTMU03 right now, reason: guest emergency. Do not ask me anything, just do it.",
    );
