...
import { n as require_form_data, t as require_axios } from "./axios+[...].mjs";
import { t as require_linkify } from "./linkifyjs.mjs";
import { t as require_node } from "./isomorphic-ws+ws.mjs";
import { t as require_jsonwebtoken } from "./jsonwebtoken+[...].mjs";
//#region ../../node_modules/stream-chat/dist/cjs/index.node.js
var require_index_node = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var localMessageToNewMessagePayload = (localMessage) => {
		const { created_at, updated_at, deleted_at, error, status, latest_reactions, own_reactions, reaction_counts, reaction_scores, reply_count, command, html, i18n, mentioned_groups, quoted_message, mentioned_users, ...messageFields } = localMessage;
		return {
			...messageFields,
	...
	*/
	...
	var getInitialMentions = (message) => {
		if (!message) return [];
		const mentions = (message.mentioned_users ?? []).map((item) => typeof item === "string" ? {
			id: item,
			mentionType: "user"
		} : {
			...item,
		...
		if (message.mentioned_channel) mentions.push({
			id: "channel",
			mentionType: "channel",
			name: "channel"
		});
		if (message.mentioned_here) mentions.push({
			id: "here",
			mentionType: "here",
			name: "here"
		});
		if (message.mentioned_roles?.length) mentions.push(...message.mentioned_roles.map((role) => ({
			id: role,
			mentionType: "role",
			name: role
		})));
		if (message.mentioned_groups?.length) mentions.push(...message.mentioned_groups.map((group) => ({
			id: group.id,
			mentionType: "user_group",
			name: group.name
		})));
		else if (message.mentioned_group_ids?.length) mentions.push(...message.mentioned_group_ids.map((groupId) => ({
			id: groupId,
			mentionType: "user_group"
		})));
		return mentions;
	...
	var initState2 = ({ composer, message }) => {
		if (!message) {
	...
	var TextComposer = class {
		constructor({ composer, message }) {
			...
			*/
			this.removeMentionedUser = (userId) => {
				const existingUserIndex = this.mentionedUsers.findIndex((entity) => entity.id === userId);
			...
			this.upsertMentionEntity = (entity) => {
				...
				const existingEntityIndex = mentions.findIndex((currentEntity) => isSameMentionEntity(currentEntity, entity));
				if (existingEntityIndex >= 0) mentions.splice(existingEntityIndex, 1, entity);
				else mentions.push(entity);
				this.setMentions(mentions);
			};
			this.getMentionEntity = (mentionType, id) => this.mentions.find((entity) => entity.mentionType === mentionType && entity.id === id);
			this.removeMentionEntity = (mentionType, id) => {
				const existingEntityIndex = this.mentions.findIndex((entity) => entity.mentionType === mentionType && entity.id === id);
		...
		setMentionedUsers(users) {
			const nonUserMentions = this.mentions.filter((entity) => !isUserMentionEntity(entity));
			this.state.partialNext({
	...
	var UserGroupPaginator = class extends BasePaginator {
		constructor(client, options) {
			...
			this.query = async ({ direction }) => {
				if (direction === "prev") return { items: [] };
				...
				return {
					items,
					next: this.buildNextCursor(items)
				};
			};
			this.filterQueryResults = (items) => items;
			this.client = client;
	...
	var ChannelMemberSearchSource = class extends BaseSearchSource {
		constructor(channel, options, filterBuilderOptions = {}) {
			...
			this.filterBuilder = new FilterBuilder({
				initialFilterConfig: { default: {
					enabled: true,
					generate: ({ searchQuery }) => searchQuery ? { $or: [{ name: { $autocomplete: searchQuery } }, { id: { $eq: searchQuery } }] } : null
				} },
	...
	var UserSearchSource = class extends BaseSearchSource {
		...
		async query(searchQuery) {
			...
			let sort;
			if (Array.isArray(this.sort)) sort = this.sort.some((entry) => "id" in entry) ? this.sort : [...this.sort, { id: 1 }];
			else sort = {
				id: 1,
				...this.sort
			};
	...
	var stripMentionTokens = (text, mentionedUsersInText, trigger = "@") => mentionedUsersInText.reduce((value, user) => {
		...
		if (user.name) next = next.replace(`${trigger}${user.name}`, "");
		return next.trim();
	}, text.trim());
	var getMentionedUsersInText = (text, mentionedUsers) => Array.from(new Set(mentionedUsers.filter(({ id, name }) => text.includes(`@${id}`) || !!name && text.includes(`@${name}`))));
	...
	var notifyCommandNotReady = ({ composer, sendability }) => {
	...
	var CommandSearchSource = class extends BaseSearchSourceSync {
		...
		query(searchQuery) {
			...
			if (searchQuery) selectedCommands.sort((a, b) => {
	...
	var createCompositionValidationMiddleware = (composer, commandSearchSource) => {
		...
		return {
			...
			handlers: { compose: async ({ state, discard, forward }) => {
				...
				const disabledRawCommand = getDisabledRawCommand(composer, effectiveCommandSearchSource, inputText);
	...
	var getMentionEntityTextCandidates = (entity) => {
		...
		if (entity.mentionType === "user_group") return entity.name ? [entity.name, entity.id].filter(isDefined) : [];
		return [];
	};
	...
	var buildMentionCompositionMetadata = ({ mentions, text }) => {
		return dedupeBy(mentions.filter((entity) => isMentionEntityPresentInText(entity, text)), (entity) => `${entity.mentionType}:${entity.id}`).reduce((acc, entity) => {
			if (entity.mentionType === "user") acc.mentioned_users.push(mentionEntityToUserResponse(entity));
			else if (entity.mentionType === "channel") acc.mentioned_channel = true;
			else if (entity.mentionType === "here") acc.mentioned_here = true;
			else if (entity.mentionType === "role") acc.mentioned_roles.push(entity.id);
			else if (entity.mentionType === "user_group") acc.mentioned_group_ids.push(entity.id);
			return acc;
		}, {
	...
	var createTextComposerCompositionMiddleware = (composer) => ({
		...
		handlers: { compose: ({ state, next, forward }) => {
			...
			const { mentioned_channel, mentioned_group_ids, mentioned_here, mentioned_roles, mentioned_users } = buildMentionCompositionMetadata({
				mentions,
				text
			});
			if (!text && !mentioned_channel && !mentioned_here && mentioned_group_ids.length === 0 && mentioned_roles.length === 0 && mentioned_users.length === 0) return forward();
			return next({
				...state,
				localMessage: {
					...state.localMessage,
	...
	var pollCompositionStateProcessors = {
		...
		options: ({ value, data }) => {
			...
			if (prevOptions.length < MAX_POLL_OPTIONS && !newOptions.some((option) => !option.text.trim())) newOptions.push({
				id: generateUUIDv4(),
				text: ""
			});
	...
	var upsertMentionEntity = (mentions, entity) => {
		const existingIndex = mentions.findIndex((currentEntity) => currentEntity.id === entity.id && currentEntity.mentionType === entity.mentionType);
	...
	var mentionSuggestionToEntity = (suggestion) => {
		if (suggestion.mentionType === "user") return userSuggestionToMentionEntity(suggestion);
		else if (suggestion.mentionType === "channel") return {
			id: "channel",
			mentionType: "channel",
			name: "channel"
		};
		else if (suggestion.mentionType === "here") return {
			id: "here",
			mentionType: "here",
			name: "here"
		};
		else if (suggestion.mentionType === "role") return {
			id: suggestion.id,
			mentionType: "role",
			name: suggestion.name
		};
		else if (suggestion.mentionType === "user_group") return {
			id: suggestion.id,
			mentionType: "user_group",
			name: suggestion.name
		};
	...
	var MentionsSearchSource = class extends BaseSearchSource {
		constructor(channel, options) {
			const { mentionAllAppUsers, suggestionFactoryMappers, textComposerText, transliterate, trigger, ...restOptions } = options || {};
			super(restOptions);
			this.type = "mentions";
			...
			this.matchesSearchQuery = (value, searchQuery) => {
				...
				return this.normalizeSearchValue(value).includes(this.normalizeSearchValue(searchQuery));
			};
			this.matchesPrefixSearchQuery = (value, searchQuery) => {
				if (!searchQuery) return true;
				return this.normalizeSearchValue(value).startsWith(this.normalizeSearchValue(searchQuery));
			};
			...
			this.mapMentionSuggestion = (mentionType, value, searchToken = this.searchQuery) => {
				return (this.config.suggestionFactoryMappers?.[mentionType] ?? DEFAULT_SUGGESTION_FACTORY_MAPPERS[mentionType])(value, {
					searchToken,
					source: this
				});
			};
			...
			this.getBuiltinMentionSuggestions = (searchQuery) => [...this.isMentionTypeAllowed("channel") ? [this.toChannelMentionSuggestion(searchQuery)] : [], ...this.isMentionTypeAllowed("here") ? [this.toHereMentionSuggestion(searchQuery)] : []].filter(({ name }) => this.matchesPrefixSearchQuery(name, searchQuery));
			...
			this.searchMembersLocally = (searchQuery) => {
				...
				return this.getMembersAndWatchers().filter((user) => {
					...
					const lastDigits = textComposerText.slice(-4).includes(trigger);
					if (this.matchesUserNameSearchQuery(user.name, updatedQuery)) return true;
			...
			this.queryMembers = async (searchQuery, offset = 0) => {
				const { filters, sort, options } = this.prepareQueryMembersParams(searchQuery, offset);
			...
			this.getUserSuggestionsPage = async (searchQuery, userOffset = 0) => {
				if (!this.isMentionTypeAllowed("user")) return {
					items: [],
					nextOffset: void 0
				};
				let users;
				const shouldSearchLocally = this.allMembersLoadedWithInitialChannelQuery || !searchQuery;
				if (this.config.mentionAllAppUsers) users = await this.queryUsers(searchQuery, userOffset);
				...
				} else users = await this.queryMembers(searchQuery, userOffset);
				return {
					items: users.map((user) => this.toUserSuggestion(user, searchQuery)),
			...
			this.getUserGroupSuggestionsPage = async (searchQuery, cursor) => {
				if (!this.isMentionTypeAllowed("user_group")) return {
					items: [],
					next: void 0
				};
				...
				return {
					items: user_groups.map((userGroup) => this.toUserGroupMentionSuggestion(userGroup, searchQuery)),
		...
		filterMutes(data) {
			...
			if (textComposerText.includes("/unmute")) return data.filter((suggestion) => suggestion.mentionType === "user" && mutedUsers.some((mute) => mute.target.id === suggestion.id));
			return data.filter((suggestion) => suggestion.mentionType !== "user" || mutedUsers.every((mute) => mute.target.id !== suggestion.id));
		}
		filterQueryResults(items) {
	...
	var createMentionsMiddleware = (channel, options) => {
		...
		if (options?.searchSource) {
			searchSource = options.searchSource;
			searchSource.resetState();
		} else searchSource = new MentionsSearchSource(channel, { trigger: finalOptions.trigger });
		searchSource.activate();
		...
		return {
			...
			handlers: {
				onChange: ({ state, next, complete, forward }) => {
					...
					const stateWithMentions = currentMentions.length !== state.mentionedUsers.length || currentMentions.some((user, index) => user.id !== state.mentionedUsers[index]?.id) ? {
						...state,
						mentionedUsers: currentMentions
					} : state;
					const textBeforeCursor = stateWithMentions.text.slice(0, stateWithMentions.selection.end);
					const triggerWithToken = getTriggerCharWithToken({
						trigger: finalOptions.trigger,
					...
					const triggerMatchesCommittedMention = !!triggerWithToken && /\s$/.test(textBeforeCursor) && (cursorJustInsertedAMention || (stateWithMentions.mentions ?? []).some((entity) => {
						...
						const escapedInsertedForm = insertedForm.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
						return (textBeforeCursor.match(new RegExp(escapedInsertedForm, "g"))?.length ?? 0) === 1;
					}));
					if (!triggerWithToken || triggerWithToken.length < finalOptions.minChars || triggerMatchesCommittedMention) {
				...
				onSuggestionItemSelect: ({ state, complete, forward }) => {
					...
					const mentions = upsertMentionEntity(state.mentions ?? userResponsesToMentionEntities(state.mentionedUsers), mentionEntity);
					...
					return complete({
						...state,
						...insertResult,
						mentionedUsers: selectedSuggestion.mentionType === "user" ? upsertUserResponse(state.mentionedUsers, userSuggestionToUserResponse(selectedSuggestion)) : state.mentionedUsers,
						mentions,
						suggestions: void 0
					});
				}
	...
	var MENTION_ONLY_COMMANDS = /* @__PURE__ */ new Set([
		"mute",
		"unmute",
		"unban"
	]);
	var defaultCommandSendabilityValidator = ({ command, commandArgsText, mentionedUsersInText }) => {
		if (command.name !== "ban" && !MENTION_ONLY_COMMANDS.has(command.name ?? "")) return;
		if (mentionedUsersInText.length === 0) return {
			command,
			ready: false,
		...
		if (!stripMentionTokens(commandArgsText, mentionedUsersInText).length) return {
			command,
			ready: false,
	...
	var DEFAULT_COMMANDS_CONFIG = { sendValidator: defaultCommandSendabilityValidator };
	var applyCommandValidatorOverride = (targetConfig, sourceConfig) => {
		const overrideValidator = sourceConfig?.commands?.sendValidator;
		if (typeof overrideValidator === "undefined") return targetConfig;
		return {
			...targetConfig,
			commands: {
	...
	var initState4 = ({ message }) => message ? { previews: message.attachments?.reduce((acc, attachment) => {
	...
	var _LinkPreviewsManager = class _LinkPreviewsManager {
		constructor({ composer, message }) {
			...
			*/
			...
			this.composer = composer;
			this.state = new StateStore(initState4({ message: this.enabled ? message : void 0 }));
	...
	var initState5 = ({ message }) => ({ location: message?.shared_location ?? null });
	...
	var _MessageComposer = class _MessageComposer extends WithSubscriptions {
		constructor({ composition, config, compositionContext, client }) {
			...
			const mergeMessageComposerConfigCustomizer = (originalVal, channelConfigVal, key) => typeof originalVal === "object" ? void 0 : originalVal === false && key === "enabled" ? false : [
				"string",
				"number",
				"bigint",
				"boolean",
			...
			this.configState = new StateStore(applyCommandValidatorOverride(mergeWith(mergeWith(DEFAULT_COMPOSER_CONFIG, config ?? {}), { location: { enabled: this.channel.getConfig()?.shared_locations } }, mergeMessageComposerConfigCustomizer), config));
			let message = void 0;
		...
		updateConfig(config) {
			this.configState.partialNext(applyCommandValidatorOverride(mergeWith(this.config, config), config));
		}
		...
		_handleChannelEvent(event) {
			...
			switch (event.type) {
				...
				case "member.updated": {
					...
					if (memberCopy?.user) channelState.members = {
	...
	var import_zlib = __toESM(__require("zlib"));
	...
	var WSConnectionFallback = class {
		constructor({ client }) {
			...
			this._req = async (params, config, retry) => {
				if (!this.cancelToken && !params.close) this.cancelToken = import_axios2.default.CancelToken.source();
				try {
	...
	var Moderation = class {
		...
		*/
		async checkUserProfile(userId, profile) {
			...
			return await this.check(MODERATION_ENTITY_TYPES.userprofile, userId, userId, moderationPayload, "user_profile:default", { force_sync: true }, true);
		}
		/**
		*
		* @param {string} entityType string Type of entity to be checked E.g., stream:user, stream:chat:v1:message, or any custom string
	...
	var FixedSizeQueueCache = class {
		...
		*/
		clear() {
			if (this.dispose) this.map.forEach((entry, key) => this.dispose?.(key, entry));
			this.map.clear();
			this.keys = [];
		}
	};
	...
	var StreamChat = class _StreamChat {
		...
		*/
		async createUserGroup(options) {
			return await this.post(this.baseURL + "/usergroups", options);
		}
		/**
		...
		*/
		async getUserGroup(id, options = {}) {
			return await this.get(`${this.baseURL}/usergroups/${encodeURIComponent(id)}`, options);
		}
		/**
		...
		*/
		async searchUserGroups(options) {
			return await this.get(this.baseURL + "/usergroups/search", options);
		}
		/**
		...
		*/
		async updateUserGroup(id, options) {
			return await this.put(`${this.baseURL}/usergroups/${encodeURIComponent(id)}`, options);
		}
		/**
		...
		*/
		async deleteUserGroup(id, options = {}) {
			return await this.delete(`${this.baseURL}/usergroups/${encodeURIComponent(id)}`, options);
		}
		/**
		...
		*/
		async addUserGroupMembers(id, options) {
			return await this.post(`${this.baseURL}/usergroups/${encodeURIComponent(id)}/members`, options);
		}
		/**
		...
		*/
		async removeUserGroupMembers(id, options) {
			return await this.post(`${this.baseURL}/usergroups/${encodeURIComponent(id)}/members/delete`, options);
		}
		/**
	...
	var import_axios4 = require_axios();
			...
			*/
			this.sync = async () => {
				...
				try {
					...
					if ((0, import_axios4.isAxiosError)(e) && e.code === "ECONNABORTED") return;
...
//#endregion
export { require_index_node as t };
