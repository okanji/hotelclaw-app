...
var catalog_default = defineDynamic({ events: { "session.started": async (_event, ctx) => {
	...
	if (grants.has("update_task")) tools.update_task = defineTool({
		...
		execute: async ({ task_id, status, priority, due_at, assignee_name, title, description }) => await __eve_dynamic_exec_29({
			propertyId,
			senderId
		}, {
			task_id,
	...
	if (grants.has("update_document")) tools.update_document = defineTool({
		...
		execute: async ({ document_id, content_html, mode }, toolCtx) => await __eve_dynamic_exec_32({
			propertyId,
			sessionChannelId
		}, {
			document_id,
	...
	if (grants.has("archive_document")) tools.archive_document = defineTool({
		...
		execute: async ({ document_id, reason }) => await __eve_dynamic_exec_33({ propertyId }, {
			document_id,
			reason
		}),
	...
	if (grants.has("search_tasks")) tools.search_tasks = defineTool({
		...
		execute: async ({ query, include_done, limit }) => await __eve_dynamic_exec_34({ propertyId }, {
			query,
			include_done,
			limit
		}),
	...
	if (grants.has("list_documents")) tools.list_documents = defineTool({
		...
		execute: async ({ title_contains, limit }) => await __eve_dynamic_exec_35({ propertyId }, {
			title_contains,
			limit
		}),
	...
	if (grants.has("list_meetings")) tools.list_meetings = defineTool({
		...
		execute: async ({ past_days, next_days, limit }) => await __eve_dynamic_exec_36({ propertyId }, {
			past_days,
			next_days,
			limit
		}),
	...
	if (grants.has("list_bookings")) tools.list_bookings = defineTool({
		...
		execute: async ({ past_days, next_days, status, limit }) => await __eve_dynamic_exec_37({ propertyId }, {
			past_days,
			next_days,
			status,
			limit
	...
	if (grants.has("search_chat_messages")) tools.search_chat_messages = defineTool({
		...
		execute: async ({ query, limit }) => await __eve_dynamic_exec_38({
			propertyId,
			senderId
		}, {
			query,
	...
	if (grants.has("list_forms")) tools.list_forms = defineTool({
		...
		inputSchema: object({ limit: number().int().min(1).max(50).default(25) }),
		execute: async ({ limit }) => await __eve_dynamic_exec_39({ propertyId }, { limit }),
	...
	if (grants.has("get_form_response_summaries")) tools.get_form_response_summaries = defineTool({
		...
		execute: async ({ form_id, limit }) => await __eve_dynamic_exec_40({ propertyId }, {
			form_id,
			limit
		}),
	...
	if (grants.has("guest_conversation_insights")) tools.guest_conversation_insights = defineTool({
		...
		execute: async ({ days, limit }) => await __eve_dynamic_exec_41({ propertyId }, {
			days,
			limit
		}),
	...
	});
	{
		...
		if (grants.has("get_insight_brief")) tools.get_insight_brief = defineTool({
			...
			inputSchema: object({}),
			execute: async () => await __eve_dynamic_exec_42({
				propertyId,
				senderId,
				ROLE_DENIED
			}),
		...
		if (grants.has("get_weekly_report")) tools.get_weekly_report = defineTool({
			...
			execute: async ({ audience }) => await __eve_dynamic_exec_43({
				propertyId,
				senderId,
				ROLE_DENIED
			}, { audience }),
		...
		if (grants.has("list_handovers")) tools.list_handovers = defineTool({
			...
			execute: async ({ limit }) => await __eve_dynamic_exec_44({
				propertyId,
				senderId
			}, { limit }),
	...
	if (grants.has("start_background_job") && sessionChannelId) tools.start_background_job = defineTool({
		...
		execute: async ({ headline, brief }, toolCtx) => await __eve_dynamic_exec_45({
			propertyId,
			senderId,
			sessionChannelId
		}, {
	...
	if (binding) {
		...
		if (grants.has("brain_capture")) tools.brain_capture = defineTool({
			...
			execute: async ({ slug, page_title, observation, source }) => await __eve_dynamic_exec_50({
				brainMcpUrl,
				brainCred
			}, {
				slug,
...
async function __eve_dynamic_exec_32(__vars, { document_id, content_html, mode }, toolCtx) {
...
async function __eve_dynamic_exec_33(__vars, { document_id, reason }) {
	const { propertyId } = __vars;
	const supabase = serviceClient();
...
async function __eve_dynamic_exec_34(__vars, { query, include_done, limit }) {
	const { propertyId } = __vars;
...
async function __eve_dynamic_exec_35(__vars, { title_contains, limit }) {
	const { propertyId } = __vars;
...
async function __eve_dynamic_exec_36(__vars, { past_days, next_days, limit }) {
	const { propertyId } = __vars;
	const now = Date.now();
...
async function __eve_dynamic_exec_37(__vars, { past_days, next_days, status, limit }) {
	const { propertyId } = __vars;
	const now = Date.now();
...
async function __eve_dynamic_exec_38(__vars, { query, limit }) {
	const { propertyId, senderId } = __vars;
	const apiKey = process.env.NEXT_PUBLIC_STREAM_API_KEY;
	const secret = process.env.STREAM_API_SECRET;
...
async function __eve_dynamic_exec_39(__vars, { limit }) {
	const { propertyId } = __vars;
	const supabase = serviceClient();
...
async function __eve_dynamic_exec_40(__vars, { form_id, limit }) {
	const { propertyId } = __vars;
	const supabase = serviceClient();
...
async function __eve_dynamic_exec_41(__vars, { days, limit }) {
	const { propertyId } = __vars;
...
async function __eve_dynamic_exec_43(__vars, { audience }) {
	const { propertyId, senderId, ROLE_DENIED } = __vars;
	const { data: sender } = await serviceClient().from("memberships").select("role").eq("property_id", propertyId).eq("user_id", senderId).maybeSingle();
	if (!sender || !["owner", "manager"].includes(sender.role)) return { denied: ROLE_DENIED };
...
async function __eve_dynamic_exec_44(__vars, { limit }) {
	const { propertyId, senderId } = __vars;
	const { data: sender } = await serviceClient().from("memberships").select("role").eq("property_id", propertyId).eq("user_id", senderId).maybeSingle();
...
async function __eve_dynamic_exec_45(__vars, { headline, brief }, toolCtx) {
...
async function __eve_dynamic_exec_50(__vars, { slug, page_title, observation, source }) {
	const { brainMcpUrl, brainCred } = __vars;
...
var channel_brain_default = defineDynamic({ events: { "session.started": async (_event, ctx) => {
	...
	return {
		brain_search: defineTool({
			description: brainToolDescriptions.brain_search,
			inputSchema: brainToolSchemas.brain_search,
			execute: async ({ query, limit }) => await __eve_dynamic_exec_16({
				brainMcpUrl,
				brainCred
			}, {
				query,
		...
		brain_think: defineTool({
			description: brainToolDescriptions.brain_think,
			inputSchema: brainToolSchemas.brain_think,
			execute: async ({ question }) => await __eve_dynamic_exec_17({
				brainMcpUrl,
				brainCred
			}, { question }),
		...
		brain_get: defineTool({
			description: brainToolDescriptions.brain_get,
			inputSchema: brainToolSchemas.brain_get,
			execute: async ({ slug }) => await __eve_dynamic_exec_18({
				brainMcpUrl,
				brainCred
			}, { slug }),
		...
		brain_list: defineTool({
			description: brainToolDescriptions.brain_list,
			inputSchema: brainToolSchemas.brain_list,
			execute: async ({ prefix, limit }) => await __eve_dynamic_exec_19({
				brainMcpUrl,
				brainCred
			}, {
				prefix,
		...
		brain_capture: defineTool({
			description: brainToolDescriptions.brain_capture,
			inputSchema: brainToolSchemas.brain_capture,
			execute: async ({ slug, page_title, observation, source }) => await __eve_dynamic_exec_20({
				brainMcpUrl,
				brainCred
			}, {
				slug,
...
async function __eve_dynamic_exec_16(__vars, { query, limit }) {
	const { brainMcpUrl, brainCred } = __vars;
	const result = await callBrainToolDirect(brainMcpUrl, brainCred, "search", {
		query,
		limit
...
async function __eve_dynamic_exec_17(__vars, { question }) {
	const { brainMcpUrl, brainCred } = __vars;
	const result = await callBrainToolDirect(brainMcpUrl, brainCred, "think", { question }, { timeoutMs: 6e4 });
	return result.ok ? { answer: result.content } : {
		unavailable: true,
...
async function __eve_dynamic_exec_18(__vars, { slug }) {
	const { brainMcpUrl, brainCred } = __vars;
	const result = await callBrainToolDirect(brainMcpUrl, brainCred, "get_page", { slug });
	if (!result.ok) return {
		unavailable: true,
...
async function __eve_dynamic_exec_19(__vars, { prefix, limit }) {
	const { brainMcpUrl, brainCred } = __vars;
	const result = await callBrainToolDirect(brainMcpUrl, brainCred, "list_pages", {
		...prefix ? { prefix } : {},
		limit,
...
async function __eve_dynamic_exec_20(__vars, { slug, page_title, observation, source }) {
	const { brainMcpUrl, brainCred } = __vars;
	if (!(await callBrainToolDirect(brainMcpUrl, brainCred, "get_page", { slug })).ok) {
		const created = await callBrainToolDirect(brainMcpUrl, brainCred, "put_page", {
			slug,
...
__eveStepRegistry$3.set("eve:dynamic-tool//__eve_dynamic_exec_16", __eve_dynamic_exec_16);
...
var pod_tools_default = defineDynamic({ events: { "session.started": async (_event, ctx) => {
	...
	if (allowed.has("create_task")) tools.create_task = defineTool({
		...
		execute: async ({ title, description, priority }) => await __eve_dynamic_exec_3({
			propertyId,
			userId
		}, {
			title,
	...
	if (allowed.has("update_task")) tools.update_task = defineTool({
		...
		execute: async ({ task_id, status, priority }) => await __eve_dynamic_exec_4({ propertyId }, {
			task_id,
			status,
			priority
		}),
	...
	if (allowed.has("read_doc")) tools.read_doc = defineTool({
		...
		inputSchema: object({ document_id: string().uuid() }),
		execute: async ({ document_id }) => await __eve_dynamic_exec_6({ propertyId }, { document_id }),
	...
	if (allowed.has("get_bookings")) tools.get_bookings = defineTool({
		...
		execute: async ({ from, days, status, limit }) => await __eve_dynamic_exec_7({ propertyId }, {
			from,
			days,
			status,
			limit
	...
	if (allowed.has("get_booking")) tools.get_booking = defineTool({
		...
		execute: async ({ reference }) => await __eve_dynamic_exec_8({ propertyId }, { reference }),
	...
	if (allowed.has("notify_channel")) tools.notify_channel = defineTool({
		...
		execute: async ({ channel_id, text }) => await __eve_dynamic_exec_9({ propertyId }, {
			channel_id,
			text
		}),
	...
	if (allowed.has("refund_booking")) tools.refund_booking = defineTool({
		...
		execute: async ({ reference, reason }) => await __eve_dynamic_exec_10({
			bot,
			propertyId,
			userId
		}, {
	...
	if (allowed.has("override_rate")) tools.override_rate = defineTool({
		...
		execute: async ({ booking_reference, description, new_rate }) => await __eve_dynamic_exec_11({
			bot,
			propertyId,
			userId
		}, {
	...
	if (allowed.has("comp_night")) tools.comp_night = defineTool({
		...
		execute: async ({ booking_reference, reason }) => await __eve_dynamic_exec_12({
			bot,
			propertyId,
			userId
		}, {
	...
	if (allowed.has("brain_query")) tools.brain_query = defineTool({
		...
		execute: async ({ query }) => await __eve_dynamic_exec_13({
			brainUrl,
			brainTokenRef
		}, { query }),
	...
	if (allowed.has("brain_get")) tools.brain_get = defineTool({
		...
		execute: async ({ path }) => await __eve_dynamic_exec_14({
			brainUrl,
			brainTokenRef
		}, { path }),
	...
	if (allowed.has("brain_write")) tools.brain_write = defineTool({
		...
		execute: async ({ path, page_title, observation, source }) => await __eve_dynamic_exec_15({
			brainUrl,
			brainTokenRef
		}, {
			path,
...
async function __eve_dynamic_exec_3(__vars, { title, description, priority }) {
	const { propertyId, userId } = __vars;
...
async function __eve_dynamic_exec_4(__vars, { task_id, status, priority }) {
	const { propertyId } = __vars;
...
async function __eve_dynamic_exec_7(__vars, { from, days, status, limit }) {
	const { propertyId } = __vars;
...
async function __eve_dynamic_exec_8(__vars, { reference }) {
	const { propertyId } = __vars;
...
async function __eve_dynamic_exec_9(__vars, { channel_id, text }) {
	const { propertyId } = __vars;
	const apiKey = process.env.NEXT_PUBLIC_STREAM_API_KEY;
	const secret = process.env.STREAM_API_SECRET;
	if (!apiKey || !secret) return { error: "Stream not configured." };
...
async function __eve_dynamic_exec_10(__vars, { reference, reason }) {
...
async function __eve_dynamic_exec_11(__vars, { booking_reference, description, new_rate }) {
	const { bot, propertyId, userId } = __vars;
	const { data, error } = await serviceClient().from("tasks").insert({
		property_id: propertyId,
...
async function __eve_dynamic_exec_12(__vars, { booking_reference, reason }) {
	const { bot, propertyId, userId } = __vars;
...
async function __eve_dynamic_exec_13(__vars, { query }) {
...
async function __eve_dynamic_exec_14(__vars, { path }) {
	const { brainUrl, brainTokenRef } = __vars;
...
async function __eve_dynamic_exec_15(__vars, { path, page_title, observation, source }) {
	const { brainUrl, brainTokenRef } = __vars;
...
//#endregion
//#region .eve/builds/mrxvybbl-fa12f349-f3ac-41dc-978b-751d83659d57/host/compiled-artifacts-bootstrap.mjs
...
//#endregion
//#region .eve/builds/mrxvybbl-fa12f349-f3ac-41dc-978b-751d83659d57/nitro/workflow/workflows-handler.mjs
...
//#endregion
//#region .eve/builds/mrxvybbl-fa12f349-f3ac-41dc-978b-751d83659d57/host/compiled-artifacts-workflow-world.mjs
