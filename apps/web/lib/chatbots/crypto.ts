import "server-only";
/**
 * At-rest encryption for custom-action header values (API keys live in
 * them). AES-256-GCM with a key derived from the app secret — ciphertext is
 * `v1.<iv>.<tag>.<data>` base64url. Not a substitute for a real KMS, but it
 * keeps bearer tokens out of plaintext DB rows, dumps, and logs.
 */
