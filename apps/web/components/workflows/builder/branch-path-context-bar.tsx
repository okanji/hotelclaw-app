...
export function BranchPathContextBar({
  ...
  return (
    <nav
      ...
      className={cn(
        "flex flex-wrap items-center gap-1 border-b border-border bg-muted/[0.06] px-4 py-2 text-xs",
        className,
      )}
    >
      <button
        ...
        className="inline-flex max-w-[min(100%,12rem)] min-w-0 items-center gap-1 rounded-md px-1 py-0.5 text-muted-foreground transition-colors hover:bg-accent"
      ...
      {onOpenPath ? (
        <button
          ...
          onClick={onOpenPath}
          className={cn(
            "rounded-md px-1 py-0.5 font-semibold transition-colors hover:bg-accent",
            pathTone,
          )}
        >
          {pathLabel}
