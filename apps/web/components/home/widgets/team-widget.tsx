    ...
    >
      {shown.map((m) => (
        <li key={m.id} className="flex items-center gap-3 px-1 py-2.5">
          ...
          <span className="shrink-0 text-xs tracking-tight text-muted-foreground">
            {ROLE_LABEL[m.role] ?? m.role}
          </span>
        </li>
      ))}
      {extra > 0 ? (
        <li className="px-1 py-2.5 text-xs tracking-tight text-muted-foreground tabular-nums">
          +{extra} more
        </li>
      ) : null}
    </ul>
