...
**Provisioning lifecycle (every property gets a brain, 2026-07-19):** new app-created properties are auto-provisioned at creation — `bootstrapProperty` (onboarding actions) calls `lib/brain/provision.ts:provisionPropertyBrain`, which drives the gbrain CLI (env-gated on `GBRAIN_HOME` + `BRAIN_MCP_URL`, binary via `GBRAIN_BIN`; active on the single-host dev topology, instant no-op on Vercel where the CLI doesn't exist; failures are fail-soft and surface as a `createWorkspace` warning). Script-seeded or missed properties are reconciled by the sweep: `node --env-file=.env.local scripts/provision-property-brain.mjs --all` (idempotent; run it on the brain host — the prod mechanism until gbrain grows an HTTP admin surface; keep the CLI command shapes in `provision.ts` and the script in sync). **Prompt honesty contract:** the channel bot's static persona (`agent-config.ts:CHANNEL_BOT_INSTRUCTIONS`) no longer claims a brain; `apps/agent/agent/instructions/dynamic.ts` appends a per-session "Knowledge brain" section for the virtual channel bot stating whether brain tools are actually mounted (binding resolved) or explicitly absent.
